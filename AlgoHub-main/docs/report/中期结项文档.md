...
spring:
  datasource:
    # MySQL 8.0 数据库连接，使用 HikariCP 连接池
    url: jdbc:mysql://localhost:3306/login_db?useSSL=false&serverTimezone=UTC&allowPublicKeyRetrieval=true
    username: root
    password: ********  # 生产环境应使用环境变量或配置中心管理
    driver-class-name: com.mysql.cj.jdbc.Driver
  jpa:
    hibernate:
      ddl-auto: none       # 禁用自动建表，由 SQL 脚本统一管理表结构
    show-sql: true         # 开发调试时打印 SQL，生产应关闭

jwt:
  secret: AlgoHub2026SecretKeyForJwtHS256Algo!!!!  # JWT HS256 签名密钥
```

**（2）JWT 认证框架 —— JwtUtil.java**

```java
@Component
public class JwtUtil {
    private final SecretKey KEY;  // HS256 签名密钥，构造时从配置注入

    public JwtUtil(@Value("${jwt.secret}") String secret) {
        this.KEY = Keys.hmacShaKeyFor(secret.getBytes());  // 生成 HMAC-SHA256 密钥
    }
    ...
    // 解析 Token，提取 Claims 中的 userId / username / role
...
**（3）JWT 拦截器与三级权限控制 —— JwtInterceptor.java + AdminController.java**

```java
// JwtInterceptor.java —— 请求拦截：提取 Authorization Header，解析 JWT 注入 request
@Component
public class JwtInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            // 将用户信息注入 request attributes，供后续 Controller 使用
...
// AdminController.java —— 三级角色层级保护（MASTER > ADMIN > STUDENT）
// roleLevel 数值越大权限越高，不可操作同级或更高级用户
...
// 禁用/启用用户：目标用户级别 >= 操作者级别时拒绝
if (roleLevel(target.getRole()) >= roleLevel(currentRole)) {
    return Result.error("无权操作：不可禁用同级或更高级别的用户");
}
// 修改角色：仅群主(MASTER)可操作，且只能将角色在 STUDENT/ADMIN 间切换
if (!"STUDENT".equals(newRole) && !"ADMIN".equals(newRole)) {
    return Result.error("参数错误：role 只能为 STUDENT 或 ADMIN");
}
```

**（4）线段树（带 Lazy 标记） —— init_algorithms.sql 内置 C++ 模板**

```cpp
// 线段树 —— 支持 O(log n) 区间修改与区间查询，使用 Lazy 标记优化
class SegTree {
    vector<ll> tree, lazy;  // tree 存储区间和，lazy 存储延迟标记
    int n;

    // 递归建树：将 arr 的区间信息构建到线段树中
    void build(int node, int l, int r, vector<ll>& arr) {
        if (l == r) { tree[node] = arr[l]; return; }  // 叶节点直接赋值
        int mid = (l + r) / 2;
    ...
    void push(int node, int l, int r) {
        if (lazy[node]) {
            tree[node] += lazy[node] * (r - l + 1);  // 更新当前节点
            if (l != r) {                             // 非叶节点传递给子节点
    ...
    // 区间加法：对 [ql, qr] 每个元素加 val
    void update(int node, int l, int r, int ql, int qr, ll val) {
        push(node, l, r);  // 访问前先下推标记
        if (ql > r || qr < l) return;  // 无交集
        if (ql <= l && r <= qr) { lazy[node] += val; push(node, l, r); return; }  // 完全包含
        int mid = (l + r) / 2;
...
**（5）Toggle 式交互设计 —— PostServiceImpl.java**

```java
// 点赞/取消点赞 —— 利用联合唯一索引 (post_id, user_id) 保证幂等
// 存在记录 → 删除（取消），不存在 → 新增（点赞），前端无需维护状态
@Override
@Transactional
public boolean toggleLike(Long postId, Long userId) {
    PostLike existing = likeRepo.findByPostIdAndUserId(postId, userId);
    if (!postRepo.existsById(postId)) return false;  // 帖子不存在
    if (existing != null) {
...
| 模块 | 完成度 | 主要成果 |
|------|--------|----------|
| 用户体系 | 95% | 注册、登录、JWT认证、个人信息管理、角色权限控制（三级）、忘记密码均已完成 |
| 算法知识库 | 90% | 分类浏览、关键词搜索、算法详情、代码复制、难度筛选、管理后台CRUD已完成；知识点关联跳转待实现 |
| 学习资源推荐 | 100% | 分类筛选、关键词搜索、资源详情、管理后台CRUD、分类管理全部完成 |
| 问答社区 | 95% | 帖子CRUD、评论回复、点赞收藏关注、举报处理、用户关注、我的帖子/收藏、管理审核全部完成 |
| 管理后台 | 100% | 用户管理、算法管理、分类管理、资源管理、举报管理、内容管理全部完成 |

**技术成果：**
- 完成了从 Vue3 到 React 18 的技术栈迁移
- 从 MyBatis-Plus 迁移至 Spring Data JPA
- 自实现了基于拦截器的 JWT 认证与三级权限控制框架（STUDENT/ADMIN/MASTER）
- 设计了自引用树形分类结构，覆盖 8 大类 53 子类
- 预置了 68 条算法数据（含完整 C++ 模板代码）和 10 条学习资源数据
- 实现了社区论坛完整功能链（7张表、20个API接口、Toggle式交互设计）
- 采用联合唯一索引 + toggle 模式实现幂等的点赞/收藏/关注交互
...
1. 密码加密方式从 MD5 迁移至 BCrypt，提升账户安全性
2. 增加用户输入的 XSS 过滤，加强对富文本内容的安全处理
3. 补全算法知识库的"知识点关联跳转"功能
4. 统一搜索接口的纯空格关键字校验逻辑
...
- 当前分支：main
- 主要提交：
  - `d91ff75` 第二第三次会议记录提交，ER图和UI流程图的PNG格式提交
  - `6969c43` 修改了社区论坛的部分已知问题（17文件，+450/-123行）
  - `a3803e6` 个人信息页面新增"我的帖子"管理功能，支持编辑和删除；帖子详情页补充编辑按钮
  - `c7d6992` 新增举报管理和资源管理后台页面，更新路由和导航（5文件，+472行）
  - `8f04950` 新增学习资源、问答社区和帖子详情前台页面（3文件，+581行）
  - `059eac7` 封装社区和资源模块API层（2文件，+117行）
  - `67570cc` 补充前端 TypeScript 类型定义（Post/Comment/PostReport/Resource等）
  - `ed3279c` 论坛社区数据库+后端接口+前端页面初步实现（27文件，+1192行）
  - `33b2f50` 完成用户模块测试，提交Bug清单
  - `8fe5254` 添加前端完整页面和组件：首页、登录注册、算法详情、个人中心、管理后台
  - `2e5f7f3` 恢复代理目标为 localhost:8080，作为默认开发配置
